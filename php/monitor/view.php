...
// ---------------------------------------------
// fixed previous month's table name dste issue
// update date: 2023/04/14 
// update by : Jhavik and James
// ---------------------------------------------
...
$app->post('/sensorRange', function($request, $response, $args) {
  ...
  $date =  date("Ym", strtotime($r->endDate));
  $predate = date("Ym", strtotime($r->startDate));
  ...
  if (!isset($session['uid'])){
    ...
    $result = $db->getQuery("SELECT * FROM current_history_data_{$uid}_{$predate} WHERE create_date>='$startDate' AND create_date<'$endDate' 
    UNION 
    SELECT * FROM current_history_data_{$uid}_{$date} WHERE create_date>='$startDate' 
    AND create_date<'$endDate' ORDER BY create_date");
